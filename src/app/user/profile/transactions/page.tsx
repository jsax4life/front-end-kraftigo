...
const getLocalizedLegacyStatus = (status: string, t: any) => {
  // Use translations for legacy statuses if needed, or fallback.
  // Actually let's just do it directly on render if possible, 
  // but to keep logic simple we'll pass t to readStatusLabel
  return legacyStatusLabel[status] || status;
};
...
const readStatusLabel = (p: Payment, t?: any) => {
  const label = p.statusLabel ?? legacyStatusLabel[p.status] ?? p.status;
  // if you want to localize these specific words you could do it here
  return label;
};
      ...
      <div className="max-w-4xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex gap-3 mb-8">
          <button className="bg-[#FF6600] text-white px-4 py-2.5 rounded-xl text-[13px] font-poppins font-medium flex items-center justify-between gap-2 shadow-sm min-w-[120px]">
            {t("last30Days")} <ChevronDown size={16} />
          </button>
          <button className="bg-[#F9FAFB] border border-[#EAECF0] text-[#1D2939] px-4 py-2.5 rounded-xl text-[13px] font-poppins font-medium flex items-center justify-between gap-2 min-w-[120px]">
            {t("allServices")} <ChevronDown size={16} />
          </button>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            <p className="text-[#667085] font-poppins">{t("noTransactions")}</p>
          </div>
        ) : (
          <div className="space-y-4">
            {payments.map((p) => {
              const isRefunded = p.status === "REFUNDED";
              const title = p.jobTitle?.trim() || t("serviceBooking");
              ...
              return (
                ...
                >
                  <div
                    className={`w-[48px] h-[48px] rounded-xl flex items-center justify-center shrink-0 overflow-hidden ${isRefunded ? "bg-[#E4E7EC]" : "bg-[#FFEFE5]"
                      }`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start mb-1">
                      <h3
                        className={`text-[14px] sm:text-[15px] font-poppins font-bold truncate pr-2 ${isRefunded ? "text-gray-500 line-through" : "text-[#1D2939]"
                          }`}
                      >
                        {title}
                      </h3>
                      <span
                        className={`text-[14px] sm:text-[15px] font-mabry font-bold shrink-0 ${isRefunded ? "text-gray-500" : "text-[#1D2939]"
                          }`}
                      >
                    ...
                    <div>
                      <span
                        className={`inline-block px-2.5 py-0.5 rounded-md text-[10px] font-poppins font-semibold ${statusStyles[p.status] ?? "bg-gray-100 text-gray-500"
                          }`}
                      >
                        {readStatusLabel(p)}
                      </span>
