  ...
  const searchArtisans = useMemo(() => {
    ...
    for (const svc of searchServiceResults) {
      ...
      byId.set(artisanId, {
        id: artisanId,
        name:
          (typeof artisan.displayName === "string" && artisan.displayName.trim()) ||
        ...
        image:
          ...
          undefined,
        categoryName:
          typeof (svc.category as Record<string, unknown> | undefined)?.name === "string"
        ...
        rating:
          typeof artisan.rating === "number" && Number.isFinite(artisan.rating)
            ...
            : undefined,
        reviewCount:
          typeof artisan.reviewCount === "number" && Number.isFinite(artisan.reviewCount)
            ? artisan.reviewCount
            : undefined,
        completedJobs:
          typeof artisan.completedJobs === "number" && Number.isFinite(artisan.completedJobs)
            ? artisan.completedJobs
            : undefined,
        isAvailable: typeof artisan.isAvailable === "boolean" ? artisan.isAvailable : undefined,
        hourlyRate:
          typeof artisan.hourlyRate === "number" && Number.isFinite(artisan.hourlyRate)
            ? artisan.hourlyRate
            : undefined,
        distanceKm:
            ...
            {searchQuery.trim() ? (
              <div className="bg-white rounded-2xl p-4 shadow-lg border border-[#0000001A]">
                ...
                <div className="mb-6">
                  ...
                  <div>
                    {isSearchLoading ? (
                      <p className="text-[14px] text-gray-500 font-poppins px-3 pb-3">Searching…</p>
                    ) : null}
                    {!isSearchLoading && searchRequestError ? (
                      <p className="text-[14px] text-red-500 font-poppins px-3 pb-3">{searchRequestError}</p>
                    ) : null}
                    {!isSearchLoading && !searchRequestError &&
                      searchServiceResults.slice(0, 5).map((service, index) => {
                      const category = (service.category ?? {}) as Record<string, unknown>;
                      const title =
                        (typeof service.title === "string" && service.title.trim()) || "Service";
                      const categoryName =
                        (typeof category.name === "string" && category.name.trim()) ||
                        title ||
                        "Service";
                      const categoryId =
                        typeof category.id === "string" && category.id.trim() ? category.id.trim() : undefined;
                      const serviceId =
                        typeof service.id === "string" && service.id.trim() ? service.id.trim() : undefined;
                      const price =
                        typeof service.price === "number" && Number.isFinite(service.price)
                          ? service.price
                          : null;
                      const priceType =
                        typeof service.priceType === "string" && service.priceType.toUpperCase() === "FLAT"
                          ? "FLAT"
                          : "HOURLY";
                      const durationMinutes =
                        ...
                        Number.isFinite(service.estimatedDurationMinutes)
                          ...
                          : null;
                      const artisan = (service.artisan ?? {}) as Record<string, unknown>;
                      const artisanName =
                        (typeof artisan.displayName === "string" && artisan.displayName.trim()) ||
                        (typeof artisan.fullName === "string" && artisan.fullName.trim()) ||
                        "Krafter";
                      ...
                      const thumb =
                        ...
                        (typeof category.imageUrl === "string" ? normSrc(category.imageUrl) : undefined);
                      return (
                        <button
                          key={index}
                          onClick={() => {
                        ...
                        >
                          <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0 relative bg-gray-100 flex items-center justify-center">
                            {thumb ? (
                              <Image src={thumb} alt={categoryName} fill className="object-cover" />
                            ) : (
                    ...
                    {!isSearchLoading && !searchRequestError && searchQuery.trim().length < 3 ? (
                ...
                <div>
                  ...
                  <div>
                    {!isSearchLoading && !searchRequestError && searchArtisans.slice(0, 5).map((artisan, index) => (
                      <button
                        key={index}
                        onClick={() => {
                      ...
                      >
                        <div className="relative w-10 h-10 rounded-lg overflow-hidden shrink-0 bg-gray-100">
                           <Image
                            src={normSrc(artisan.image) ?? "/images/pro.jpg"}
                            alt={artisan.name}
                            fill
                            className="object-cover"
                          />
                        ...
                        <div className="min-w-0 flex-1 text-left">
                          ...
                          <p className="text-[12px] font-poppins text-gray-500 truncate">
                            ...
                            {artisan.distanceKm != null ? ` · ${artisan.distanceKm.toFixed(1)} km away` : ""}
                          </p>
                          <p className="text-[12px] font-poppins text-gray-600 mt-0.5">
                            {artisan.rating != null ? `★ ${artisan.rating.toFixed(1)}` : "★ —"}
                            {artisan.reviewCount != null ? ` (${artisan.reviewCount})` : ""}
                            {artisan.completedJobs != null ? ` · ${artisan.completedJobs} jobs` : ""}
                            {artisan.hourlyRate != null ? ` · €${artisan.hourlyRate.toFixed(2)}/hr` : ""}
                            {artisan.isAvailable === true ? " · Available" : ""}
                          </p>
                        </div>
                      </button>
                    ))}
