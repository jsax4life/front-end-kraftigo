...
async function runConfirmCardPayment(
  ...
  clientSecret: string,
  returnUrl: string,
  opts: { paymentMethodId: string } | { card: import("@stripe/stripe-js").StripeCardElement; name: string },
): Promise<boolean> {
  const payload =
  ...
  const { error, paymentIntent } = await stripe.confirmCardPayment(clientSecret, payload);
  ...
  return (
    <div className="space-y-4">
      <p className="text-[13px] font-poppins text-gray-600">
        This card is only used for this payment. It does not replace your saved cards unless you add it in payment
        settings later.
      </p>
      {!stripe && (
        <p className="text-[12px] font-poppins text-gray-500">Connecting to Stripe…</p>
      )}
      <div>
        <label className="block text-[14px] font-poppins font-bold text-gray-900 mb-2">Name on card</label>
        <input
          type="text"
      ...
      <div>
        <label className="block text-[14px] font-poppins font-bold text-gray-900 mb-2">Card</label>
        {!elementReady && stripe && (
          <p className="text-[12px] font-poppins text-gray-500 mb-2">Loading card field…</p>
        )}
  ...
  return (
    <div className="relative bg-white w-full max-h-[90vh] rounded-t-2xl p-4 sm:p-6 pb-8 flex flex-col shadow-xl pointer-events-auto">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-[18px] sm:text-[20px] font-poppins font-bold text-gray-900">Confirm payment</h3>
        <button
          type="button"
          onClick={onCancel}
      ...
      <p className="text-[13px] font-poppins text-amber-900 bg-amber-50 border border-amber-100 rounded-xl px-3 py-2 mb-4">
        No saved card on file. Add your card below to authorize this booking, or save a card from your profile first.
      </p>
      <NewCardFields clientSecret={clientSecret} returnUrl={returnUrl} onSuccess={onSuccess} />
      <button
        type="button"
        onClick={onCancel}
...
function SavedOrNewCheckout({
  ...
  const selectedRow = useMemo(
    () => (choice === NEW_CARD_CHOICE ? null : savedMethods.find((m) => m.id === choice) ?? null),
    [choice, savedMethods],
  );
  ...
  const handleChoiceChange = (next: string) => {
    ...
    void patchBookingCheckoutPaymentMethod(bookingId, { savedPaymentMethodId: pmId })
      .then((r) => {
  ...
  return (
    <div className="relative bg-white w-full max-h-[90vh] rounded-t-2xl p-4 sm:p-6 pb-8 flex flex-col shadow-xl pointer-events-auto overflow-y-auto no-scrollbar">
      ...
      <p className="text-[13px] font-poppins text-gray-600 mb-4">
        Authorize the hold for this booking. Use a saved card or enter a different card if you prefer (for example if
        your saved card has no available balance).
      </p>
        ...
        {savedMethods.map((m) => (
          <label
            ...
            className={`flex items-center gap-3 rounded-xl border p-3 cursor-pointer transition-colors ${
              choice === m.id ? "border-brand-orange bg-orange-50/50" : "border-gray-200 hover:border-gray-300"
            }`}
          >
            <input
              type="radio"
            ...
            />
            <span className="text-[14px] font-poppins text-gray-900">{labelFor(m)}</span>
          </label>
        ))}
        <label
          className={`flex items-center gap-3 rounded-xl border p-3 cursor-pointer transition-colors ${
            choice === NEW_CARD_CHOICE ? "border-brand-orange bg-orange-50/50" : "border-gray-200 hover:border-gray-300"
          }`}
        >
          <input
            type="radio"
            ...
            className="w-4 h-4 accent-brand-orange shrink-0"
          />
          <span className="text-[14px] font-poppins font-semibold text-gray-900">Use a different card</span>
        </label>
      </fieldset>
      ...
      {choice !== NEW_CARD_CHOICE && selectedRow && (
        <div className="space-y-4">
          ...
          <p className="text-[12px] font-poppins text-gray-500">
            Your bank may still ask you to confirm this payment (for example 3D Secure).
          </p>
      ...
      {choice === NEW_CARD_CHOICE && (
        <Elements key={`${clientSecret}-new-card`} stripe={stripe} options={{ clientSecret }}>
          <NewCardFields clientSecret={clientSecret} returnUrl={returnUrl} onSuccess={onSuccess} />
          <button
            type="button"
            onClick={onCancel}
...
export default function BookingPaymentConfirmModal({
  ...
  const [stripeJs, setStripeJs] = useState<Stripe | null | undefined>(undefined);
  ...
  const readyForCheckout =
    Boolean(clientSecret && elementsOptions && !stripeConfigError && stripeJs && paymentMethodsReady);
      ...
      <div className="relative w-full sm:max-w-md z-[131] pointer-events-auto">
        ...
        {loadError && !clientSecret && (
          <div className="bg-white rounded-t-2xl sm:rounded-2xl p-6 space-y-4">
            <p className="text-[14px] font-poppins text-gray-800">{loadError}</p>
            <button
              type="button"
              onClick={onClose}
        ...
        {clientSecret && elementsOptions && stripeConfigError && (
          <div className="bg-white rounded-t-2xl sm:rounded-2xl p-6 space-y-4">
            <p className="text-[14px] font-poppins text-gray-800">{stripeConfigError}</p>
            <button
              type="button"
              onClick={onClose}
              className="w-full py-3 rounded-xl bg-gray-100 font-poppins font-semibold text-[15px]"
        ...
        {readyForCheckout && savedMethods.length === 0 && stripeJs && (
          <Elements key={clientSecret!} stripe={stripeJs} options={elementsOptions!}>
            <NewCardStandalone
              clientSecret={clientSecret!}
              returnUrl={returnUrl}
              onSuccess={handleSuccess}
