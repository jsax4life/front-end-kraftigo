  ...
  return (
    <>
      <div className="fixed inset-x-0 top-0 bottom-[96px] z-60 bg-[#F9FAFB] flex flex-col animate-in slide-in-from-bottom duration-300">
        ...
        <div
          className={`px-5 py-2.5 flex flex-col items-center gap-1 shadow-sm ${conversation.isLocked ? "bg-[#667085]" : "bg-[#FF6600]"
            }`}
        >
          <span className="text-[11px] font-poppins font-bold text-white text-center">
            {contextLabel}
            {conversation.isLocked ? t("readOnly") : ""}
          </span>
        ...
        <div className="flex-1 overflow-y-auto px-4 space-y-6 flex flex-col pt-6 pb-6 scrollbar-hide">
          ...
          {[...displayMessages].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()).map((msg, index, array) => {
            ...
            return (
              ...
              >
                <div className={`max-w-[75%] rounded-[12px] px-4 py-3 text-[14px] font-poppins leading-relaxed shadow-sm ${isMe
          ...
          {showAttachments && !conversation.isLocked && (
            <div className="grid grid-cols-3 gap-4 pt-6 pb-2 animate-in slide-in-from-bottom duration-200">
              <button className="flex flex-col items-center gap-2 group">
                ...
                <span className="text-[12px] font-poppins text-[#444444] font-medium">{t("camera")}</span>
              </button>
              <button className="flex flex-col items-center gap-2 group">
                ...
                <span className="text-[12px] font-poppins text-[#444444] font-medium">{t("photos")}</span>
              </button>
              <button
                onClick={() => {
              ...
              >
                ...
                <span className="text-[12px] font-poppins text-[#444444] font-medium">{t("location")}</span>
              </button>
            </div>
          )}
        </div>
